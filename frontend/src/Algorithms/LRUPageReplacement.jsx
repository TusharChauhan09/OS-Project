...
const LRUPageReplacement = () => {
  ...
  const runLRU = () => {
    ...
    // Track the last used time for each page
    let recentlyUsed = {};
    ...
    pages.forEach((page, index) => {
      ...
      if (currentFrames.includes(page)) {
        ...
        if (currentFrames.includes('-')) {
          ...
          // Find the page with the oldest (smallest) timestamp
      ...
      // Update timestamp for the current page
      ...
      // Update the recentlyUsed for next iteration
      recentlyUsed = updatedRecentlyUsed;
    });
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-gray-50 rounded-xl shadow-lg">
      <h2 className="text-2xl font-bold text-center mb-6 text-purple-800">LRU Page Replacement Algorithm</h2>
            ...
            <div className="mb-8">
              {currentStep > 0 && (
                <p className="text-center mb-4 text-gray-700">
                  Current Page: <span className="font-bold text-purple-700">{simulationMatrix[currentStep].page}</span>
                </p>
              )}
              {renderFrames()}
            </div>
